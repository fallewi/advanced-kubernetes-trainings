...
??? example "`kubebuilder init` - Initialize a new operator project"
    ...
    **Description:** Scaffolds the complete project skeleton: `cmd/main.go`, `Makefile`, `go.mod`, base Kustomize configs, and `.gitignore`.
    ...
    - `--plugins=go/v4` (default) uses the latest stable Go plugin
        ...
        ```bash
        # Initialize with domain liora.io
        kubebuilder init \
            --domain liora.io \
            --repo liora.io/webapp-operator
...
??? example "`make manifests` - Generate CRD and RBAC manifests"
    ...
    **Description:** Runs `controller-gen` to generate CRD YAML, RBAC ClusterRole, and webhook manifests from Go markers. Must be run after every change to markers in `*_types.go` or `*_controller.go`.
        ...
        cat config/crd/bases/apps.liora.io_webapps.yaml
...
??? example "`make install` - Install CRDs into the cluster"
    ...
    **Description:** Applies the generated CRD manifests to the currently active cluster using `kubectl apply`. After this, `kubectl get webapps` will work.
        ...
        kubectl get crds | grep liora

        # Describe the CRD schema
        kubectl describe crd webapps.apps.liora.io
...
import (
	...
	webappv1 "liora.io/webapp-operator/api/v1"
)
...
//
//+kubebuilder:rbac:groups=apps.liora.io,resources=webapps,verbs=get;list;watch;create;update;patch;delete
//+kubebuilder:rbac:groups=apps.liora.io,resources=webapps/status,verbs=get;update;patch
//+kubebuilder:rbac:groups=apps.liora.io,resources=webapps/finalizers,verbs=update
...
# Verify
kubectl get crds | grep liora
kubectl describe crd webapps.apps.liora.io | grep -A 20 "COLUMNS\|Validation"
```
...
```go
const webappFinalizer = "apps.liora.io/finalizer"
```
...
func (r *WebApp) validateWebApp() (admission.Warnings, error) {
    ...
    if len(errs) > 0 {
        return nil, apierrors.NewInvalid(
            schema.GroupKind{Group: "apps.liora.io", Kind: "WebApp"},
            r.Name,
            errs,
        )
    }
...
# Expected error:
# Error from server (WebApp.apps.liora.io "invalid-webapp" is invalid):
...
| Goal | Command |
|------|---------|
| Init new project | `kubebuilder init --domain liora.io --repo liora.io/my-op` |
...
| View all API resources | `kubectl api-resources --api-group=apps.liora.io` |
