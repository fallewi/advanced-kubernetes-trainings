...
- As completed in the previous lab, create the desired namespace [liora]:
...
kubectl create deployment -n liora nginx --image=nginx:1.17 --save-config
```
...
```sh
kubectl scale deployment -n liora nginx --replicas=3
...
kubectl get services -n liora -o wide
...
kubectl set image deployment -n liora nginx nginx=nginx:1.16 --record
...
```sh
kubectl rollout history deployment nginx -n liora
...
kubectl rollout history deployment nginx -n liora --revision=<X>  # replace here
