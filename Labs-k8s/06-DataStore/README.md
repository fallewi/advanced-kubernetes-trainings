...
kubectl delete namespace liora --ignore-not-found

# Create the desired namespace [liora]
kubectl create namespace liora
```
...
kubectl exec -it -n liora \
  $(kubectl get pod -n liora -l name=liora-secrets -o jsonpath='{.items[0].metadata.name}') \
...
!!! info "Why not use the Service?"
    ...
    In a real environment you would use the service DNS name: `liora-secrets.liora.svc.cluster.local:5000`
...
kubectl get secrets,cm -n liora

# View the secret details (note: data is Base64-encoded)
kubectl describe secret token -n liora

# View the config map details (note: data is plain text)
kubectl describe cm language -n liora
```
...
kubectl get secret token -n liora -o jsonpath='{.data.TOKEN}'

# Decode it
kubectl get secret token -n liora -o jsonpath='{.data.TOKEN}' | base64 -d
...
kubectl rollout status deployment/liora-secrets -n liora
...
kubectl delete secret token -n liora --ignore-not-found
kubectl apply -n liora -f resources/secret.yaml
...
POD=$(kubectl get pod -n liora -l name=liora-secrets -o jsonpath='{.items[0].metadata.name}')

# View secret and config files
kubectl exec -it -n liora "$POD" -- sh -c  \
  "echo '--- ConfigMap file ---';               \
...
kubectl rollout restart deployment/liora-secrets -n liora
