...
```sh
# Network tools pod
kubectl create deployment -n liora multitool --image=praqma/network-multitool
deployment.apps/multitool created

# nginx pod
kubectl create deployment -n liora nginx --image=nginx
...
kubectl exec -it <pod name> -n liora -- sh
```
...
bash-5.0# curl -s nginx.liora.svc.cluster.local
```
...
kubectl delete svc nginx -n liora
...
kubectl expose deployment -n liora nginx --port 80 --type NodePort
service/nginx exposed
...
kubectl get svc nginx -n liora -o jsonpath='{.spec.ports[0].nodePort}{"\n"}'
32593
```
...
kubectl expose deployment nginx -n liora --port 80 --type LoadBalancer
service/nginx exposed
