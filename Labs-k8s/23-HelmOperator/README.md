...
- How to create a Helm-based operator using `operator-sdk`
- How to customize the operator logic and deploy it to a cluster
...
- Docker installed (for building operator images)
- `operator-sdk` CLI installed (steps below)
...
- This creates the `nginx-operator` project specifically for watching the `Nginx` resource with APIVersion `demo.liora.co.il/v1alpha1` and Kind `Nginx`.
...
- Let’s update the value to 3 - `replicaCount: 3`.
  ...
  apiVersion: demo.liora.co.il/v1alpha1
  kind: Nginx
  metadata:
    name: nginx-sample
  spec:
...
```sh
kubectl describe nginxes.demo.liora.co.il
```
