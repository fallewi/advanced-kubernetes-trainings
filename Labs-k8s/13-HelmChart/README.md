...
##### liora-helm-demo Helm Chart structure
...
- Before you can use the `liora-helm-demo` chart, you'll need to **install** `Helm` on your local machine.
...
- Creating our custom `liora-helm-demo` `Helm` chart
- The custom `liora-helm-demo` `Helm` chart is build upon the following K8S resources:
    - ConfigMap
    - Deployment
    - Service
...
```sh
helm package liora-helm-demo
```

- This command will create a file called `liora-helm-demo-<version>.tgz` inside your current directory.
...
```sh
helm lint liora-helm-demo

## Expected output:
## ==> Linting liora-helm-demo
...
- Install the `liora-helm-demo` chart into Kubernetes cluster
...
helm install liora-helm-demo liora-helm-demo-0.1.0.tgz --dry-run
```

```sh
# Install the packed helm chart
helm install liora-helm-demo liora-helm-demo-0.1.0.tgz
```
...
helm status liora-helm-demo

# Get the rendered manifests of the release
helm get manifest liora-helm-demo

# Get the values used by the release
helm get values liora-helm-demo

# Check the resources
kubectl get all -n liora
```

### Step 07 - Test the service

- Perform an `HTTP GET` request, send it to the newly created cluster service.
- Confirm that the response contains the `liora Helm Demo` message passed from the `values.yaml` file.
...
kubectl run busybox         \
        ...
        -it                 \
        --restart=Never     \
        -- /bin/sh -c "wget -qO- http://liora-helm-demo.liora.svc.cluster.local"
...
kubectl run busybox --image=busybox --rm -it --restart=Never \
  -- /bin/sh -c "wget -qO- http://liora-helm-demo.liora.svc.cluster.local/release/name"

# Get the release revision
kubectl run busybox --image=busybox --rm -it --restart=Never \
  -- /bin/sh -c "wget -qO- http://liora-helm-demo.liora.svc.cluster.local/release/revision"
```
...
```sh
helm history liora-helm-demo

### Sample output
REVISION        UPDATED    STATUS          CHART                           APP VERSION     DESCRIPTION
1               ...        superseded      liora-helm-demo-0.1.0      1.19.7          Install complete
2               ...        deployed        liora-helm-demo-0.1.0      1.19.7          Upgrade complete
```
...
- Rollback the `liora-helm-demo` release to previous version:

```sh
helm rollback liora-helm-demo
...
helm install custom-demo liora-helm-demo-0.1.0.tgz -f custom-values.yaml

# Verify the replica count
kubectl get deployment -n liora

# Verify the message
kubectl run busybox --image=busybox --rm -it --restart=Never \
  -- /bin/sh -c "wget -qO- http://custom-demo.liora.svc.cluster.local"
...
rm liora-helm-demo/templates/broken.yaml
```
...
Upgrade the `liora-helm-demo` release to use `3` replicas, image tag `1.21.0`, and the message `"Multi-set Override"` - all in a single command.
...
helm upgrade liora-helm-demo liora-helm-demo-0.1.0.tgz \
  --set replicaCount=3 \
...
kubectl get deployment -n liora -o wide
```
...
Modify the `liora-helm-demo` chart to add an optional `Ingress` resource that is only created when `ingress.enabled` is set to `true` in `values.yaml`.
...
cat <<EOF >> liora-helm-demo/values.yaml
...
# Save this as liora-helm-demo/templates/Ingress.yaml:
{{- if .Values.ingress.enabled }}
apiVersion: networking.k8s.io/v1
kind: Ingress
metadata:
...
helm template liora-helm-demo | grep -A5 "kind: Ingress"

# 4. Verify: template WITH ingress enabled (should include Ingress resource)
helm template liora-helm-demo --set ingress.enabled=true | grep -A20 "kind: Ingress"

# 5. Cleanup - remove the ingress template
rm liora-helm-demo/templates/Ingress.yaml
```
...
helm template liora-helm-demo --set team="devops" | grep -A5 "annotations"
```
...
helm template liora-helm-demo | grep -A15 "env:"
...
Add a Helm test to the `liora-helm-demo` chart that verifies the service is reachable and returns the expected message.
...
mkdir -p liora-helm-demo/templates/tests
```
...
spec:
  containers:
    - name: wget
      ...
      args:
        - |
          RESPONSE=$(wget -qO- http://{{ include "webserver.fullname" . }}.liora.svc.cluster.local)
...
helm upgrade --install liora-helm-demo liora-helm-demo-0.1.0.tgz

# 4. Run the test
helm test liora-helm-demo

# Expected output:
# NAME: liora-helm-demo
# ...
# Phase: Succeeded
```
...
Add a `pre-install` hook to the `liora-helm-demo` chart that creates a Job to print a banner message before the main resources are installed.
...
kubectl get jobs -n liora

# Cleanup
helm uninstall liora-helm-demo
rm liora-helm-demo/templates/pre-install-hook.yaml
```
...
helm diff upgrade liora-helm-demo liora-helm-demo-0.1.0.tgz \
...
Add a `NOTES.txt` file to the `liora-helm-demo` chart that displays the release name, namespace, and instructions for testing the service after installation.
...
To test the service, run:
  ...
  kubectl run busybox --image=busybox --rm -it --restart=Never \
    -- /bin/sh -c "wget -qO- http://{{ include "webserver.fullname" . }}.liora.svc.cluster.local"
...
- To remove all resources created by this lab, uninstall the `liora-helm-demo` release:
...
```sh
kubectl delete namespace liora
```
...
```sh
kubectl get pods -n liora
kubectl describe pod <pod-name> -n liora
kubectl logs <pod-name> -n liora
```
...
```sh
kubectl get jobs -n liora
kubectl logs job/<job-name> -n liora
```
