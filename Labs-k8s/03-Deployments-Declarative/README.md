...
```sh
kubectl apply -n liora -f nginx.yaml --record=true
...
```sh
kubectl get deployments -n liora
...
```sh
kubectl get pods -n liora --watch
```
...
```sh
kubectl get pods --watch -n liora
...
```sh
kubectl scale -n liora --replicas=1 deployment/nginx
```
...
```sh
kubectl delete deployment nginx -n liora
